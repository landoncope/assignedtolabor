...
- Branding: interim mark is a gold A on blue (`src/components/Mark.tsx`, `src/app/icon.svg`,
  `apple-icon.tsx`); social preview is generated by `src/app/opengraph-image.tsx`
  (`twitter-image.tsx` re-exports it); `manifest.ts` for add-to-home-screen. A real
  logo is coming from Landon via Midjourney; swap the mark in those four places.
